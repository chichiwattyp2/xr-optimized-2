/* aframe-audioanalyser-component.js */
/* eslint-disable no-var */
if (typeof AFRAME === 'undefined') {
  throw new Error('Component attempted to register before AFRAME was available.');
}

// Single audio context (shared across all instances).
var context;

/**
 * Audio visualizer system for A-Frame. Share AnalyserNodes between components that share `src`.
 */
AFRAME.registerSystem('audioanalyser', {
  ...
  // Explicit creator used by components with {unique: true}
...
AFRAME.registerComponent('audioanalyser', {
  ...
  tick: function () {
    ...
    if (data.enableVolume || data.enableBeatDetection) {
      var sum = 0;
      for (var i = 0; i < this.levels.length; i++) { sum += this.levels[i]; }
    ...
    if (data.enableBeatDetection) {
      ...
      var volume = this.volume; // fix: don’t leak a global

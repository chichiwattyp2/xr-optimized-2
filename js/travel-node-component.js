AFRAME.registerComponent("travel-node", {
  ...
  onClick: function() {
    ...
    targetPosition.setFromMatrixPosition(
      this.travelTarget.object3D.matrixWorld
    );
    ...
    switch (this.data.transition) {
      ...
      case "move":
        if (AFRAME.components["animation"]) {
          ...
          console.warn(
            "To use the 'move' transition, you will need to add the aframe-animation-component component, available at https://github.com/ngokevin/aframe-animation-component"
          );
          this.jumpToTarget();
        }
        break;
  ...
  moveToTarget: function() {
    ...
    this.travelTarget.setAttribute(
      "animation__travel-node_movetotarget",
      attribute
    );

    this.travelTarget.addEventListener(
      "animation__travel-node_movetotarget-complete",
      function() {
        this.travelTarget.removeAttribute(
          "animation__travel-node_movetotarget"
        );
      }.bind(this)
    );
...
AFRAME.registerComponent("travel-node-defaults", {
  ...
  checkElement: function() {
    if (this.el.tagName !== "A-SCENE") {
      console.warn(
        "travel-node-defaults should be placed on the a-scene element."
      );
    }
  }
});
